...
func collectionCommand() *cli.Command {
	return &cli.Command{
		...
		Description: "Resolve ancestry with `zot collection path`, or create, rename, and delete collections with local writes. To browse the collection tree and keys, use the read-only `zot collections` command.",
