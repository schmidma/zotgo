...
func (c *Client) LibraryVersion(ctx context.Context, lib LibraryRef) (int, error) {
	_, page, err := c.do(ctx, itemsPath(c.profile, lib, ItemsOptions{}), url.Values{"limit": {"1"}})
	if err != nil {
		return 0, err
	}
...
// KeyWriteResult is a narrow write response that decodes only created keys,
// avoiding unrelated Zotero envelope fields.
...
func (c *Client) writeBatchResponse(ctx context.Context, path string, objs []json.RawMessage) ([]byte, error) {
	if len(objs) == 0 {
		return nil, nil
	}
...
// CreateItemsReturningKeys creates items while decoding only their generated keys.
func (c *Client) CreateItemsReturningKeys(ctx context.Context, lib LibraryRef, items []json.RawMessage) (KeyWriteResult, error) {
	if len(items) == 0 {
		return KeyWriteResult{}, nil
	}
	body, err := c.writeBatchResponse(ctx, c.profile.LibraryPrefix(lib)+"/items", items)
	if err != nil {
		return KeyWriteResult{}, err
	}
	return parseKeyWriteResult(body)
}
